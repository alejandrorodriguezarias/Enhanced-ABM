__includes["SocialNetwork.nls" "MakeAgents.nls" "Util.nls" "CriticalNodesMethods.nls" "GisDataMethods.nls" "CitizenMethods.nls" "ScenarioMethods.nls" "Communication.nls" "ACL.nls" "BDI.nls"]
globals[month year initial-year plot-highlight scenario1-event scenario3-event1 scenario3-event2 scenario3-event3 csv-info mod-curve mod-pos-curve]
breed [humats humat]
...
breed [counters counter];tick manager
...
to go
  ...
  let humatticks floor (ticks / 15)
  if humatticks > [last-tick] of first sort counters [
    set month Get-Month
    set year Get-Year
    ask counters [set last-tick floor (ticks / 15)]
    ...
    if month = 6 and year = 2013[
      ...
      file-open word "comm" run-number
      ask critical-nodes with [node-type = "City Council"] [
        file-print num-comm
        show num-comm
      ]
      file-close
...
to-report Need-Similarity [need-evaluation-BA-ego need-evaluation-BA-alter need-importance-ego need-importance-alter]
  ...
  [report alfa * (1 - abs(need-importance-ego - need-importance-alter))]
  [report 0]
end
...
to-report Further-Comparison-Needed? [comparison-dimension-A comparison-dimension-B theoretical-range]
  let value 0
  ifelse (comparison-dimension-A > comparison-dimension-B - gamma * theoretical-range) and (comparison-dimension-A < comparison-dimension-B + gamma * theoretical-range) [Set value true] [Set value false] ;0.1
  report value
end
...
setup</setup>
  ...
  <experiment name="experiment2+3" repetitions="1" runMetricsEveryStep="true">
    <setup>setup-landscape
setup-citizens
setup-social-network
setup</setup>
  ...
  <experiment name="experimentSinCriticos" repetitions="1" runMetricsEveryStep="true">
    <setup>setup-landscape
setup-citizens
setup-social-network
setup</setup>
  ...
  <experiment name="SensibilidadAgentes" repetitions="1" runMetricsEveryStep="true">
    <setup>setup-landscape
setup-citizens
setup-social-network
setup</setup>
    ...
    <enumeratedValueSet variable="agent-div">
      <value value="12"/>
    </enumeratedValueSet>
  ...
  <experiment name="SensibilidadAgentes2" repetitions="1" runMetricsEveryStep="true">
    <setup>setup-landscape
setup-citizens
setup-social-network
setup</setup>
